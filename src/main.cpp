...
void pomeranjeSvestenica(Camera_Movement direction);
...
bool poajavaJagnje=false;

glm::vec3 krstPoz(0.0f, 0.0f, 0.0f);
glm::vec3 svestenicaPoz(0.0f, -0.5f, 7.0f);
...
int main()
{
    ...
    unsigned int transparentTexture = loadTexture(FileSystem::getPath("resources/textures/ograda.png").c_str());
    unsigned int diffuseMap = loadTexture(FileSystem::getPath("resources/textures/zemlja/zemljica.jpg").c_str());
    unsigned int specularMap = loadTexture(FileSystem::getPath("resources//textures/zemlja/normal.jpg").c_str());
    lightShader.use();
    ...
    unsigned int floorDiffuseMap = loadTexture(FileSystem::getPath("resources/textures/zemlja/zemljica.jpg").c_str());
    unsigned int floorSpecularMap = specularMap;
    vector<glm::vec3> zemlja
            {
                    glm::vec3(-25.5f, 2.5f, -22.0f),
    ...
    vector<std::string> skyboxSides = {
        FileSystem::getPath("resources/textures/skybox/negx.png"),
        FileSystem::getPath("resources/textures/skybox/posx.png"),
        FileSystem::getPath("resources/textures/skybox/posy.png"),
        FileSystem::getPath("resources/textures/skybox/negy.png"),
        FileSystem::getPath("resources/textures/skybox/negz.png"),
        FileSystem::getPath("resources/textures/skybox/posz.png")
    };
    ...
    Model krst(FileSystem::getPath("resources/objects/ve1p5zexdlog-Cross/OBJ/Cross.obj"));
    krst.SetShaderTextureNamePrefix("material.");

    Model jagnje(FileSystem::getPath("resources/objects/Lamb_v1_L2.123ce2e26608-af3b-4ed1-983d-aa231da98620/12955_Lamb_v1_l2.obj"));
    jagnje.SetShaderTextureNamePrefix("material.");

    Model svestenica(FileSystem::getPath("resources/objects/pilgrim_female_V2_L2.123c1d75dc89-654a-477a-9835-1e13af384daf/11554_pilgrim_female_V2_L2.obj"));
    svestenica.SetShaderTextureNamePrefix("material.");

    Model house(FileSystem::getPath("resources/objects/31-village-house/medieval house.obj"));
    house.SetShaderTextureNamePrefix("material.");

    float time=glfwGetTime();

    glm::mat4* matrica;
    matrica = new glm::mat4[7];

    for (unsigned int i = 0; i < 7; i++)
    {
        glm::mat4 model = glm::mat4(1.0f);
        float x= krstPoz.x + r * cos(glm::radians(180.0 / 7 * i));
        float y=0.0f;
        float z= krstPoz.z + r * sin(glm::radians(180.0 / 7 * i));
    ...
    // lighting info
    // -------------
    glm::vec3 lightPos = glm::vec3(9.0, 4.0, 1.0);
    glm::vec3 lightColor = glm::vec3(6.0f, -2.2f, 3.8f);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if(ssaoButton==true){
          ...
          //  model = glm ::rotate (model, glm::vec3(0.0f, 1.0f, 0.0f));
            ...
            //svestenica
            model = glm::mat4(1.0f);
            model = glm::translate(model, svestenicaPoz);
            if (back)
            ...
            shaderGeometryPass.setMat4("model", model);
            svestenica.Draw(shaderGeometryPass);

            model = glm::mat4(1.0f);
            if (poajavaJagnje) {
                for (unsigned int i = 0; i < 7; i++) {
                    shaderGeometryPass.setMat4("model", matrica[i]);
                    jagnje.Draw(shaderGeometryPass);
                }
            }
            glBindFramebuffer(GL_FRAMEBUFFER, 0);
            ...
            lightShader.setVec3("dirLight.direction", 1.0f, -0.3f, 0.0f);
            lightShader.setVec3("dirLight.ambient", 0.6f, 0.5f, 0.5f);
            lightShader.setVec3("dirLight.diffuse", 0.1f, 0.8f, 0.0f);
            lightShader.setVec3("dirLight.specular", 1.0f, 1.0f, 1.0f);

            // point light setup
            lightShader.setVec3("pointLight.position", krstPoz);
            ...
            lightShader.setFloat("pointLight.linear", 0.08f);
            lightShader.setFloat("pointLight.quadratic", 0.015f);


            // spotlight setup
            lightShader.setVec3("spotLight.position", glm::vec3(0.0f, 20.0f, 0.0f));
            lightShader.setVec3("spotLight.direction", glm::vec3(0, -1, 0));
            lightShader.setVec3("spotLight.ambient", (sin(10 * time) + 1) / 2.0, 0.0f, 0.0f);
            lightShader.setVec3("spotLight.diffuse", 0.0f, 0.0f, 0.0f);
            lightShader.setVec3("spotLight.specular", (sin(10 * time) + 1) / 2.0, 0.0f, 0.0f);
            ...
            if (poajavaJagnje) {
                for (unsigned int i = 0; i < 3; i++) {
                   lightShader.setMat4("model", matrica[i]);
                    jagnje.Draw(lightShader);
                }
            }
            ...
            for (unsigned int i = 0; i < zemlja.size(); i++) {
                model = glm::mat4(1.0f);
                model = glm::translate(model, zemlja[i]);
                if (i >= zemlja.size() / 2)
...
void processInput(GLFWwindow *window) {
    ...
    if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS) {
        ...
        pomeranjeSvestenica(FORWARD);
    }
    if (glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS) {
        back = true;
        front = on_left = on_right = false;
        pomeranjeSvestenica(BACKWARD);
    }
    if (glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS) {
        on_left = true;
        on_right = front = back = false;
        pomeranjeSvestenica(LEFT);
    }
    if (glfwGetKey(window, GLFW_KEY_RIGHT) == GLFW_PRESS) {
        on_right=true;
        on_left=front=back=false;
        pomeranjeSvestenica(RIGHT);
    }
...
void pomeranjeSvestenica(Camera_Movement direction)
{
    ...
    if (direction == FORWARD)
        svestenicaPoz += camera.Front * v * s;
    if (direction == BACKWARD)
        svestenicaPoz-= camera.Front * v * s;
    if (direction == LEFT)
        svestenicaPoz -= camera.Right * v * s;
    if (direction == RIGHT)
        svestenicaPoz += camera.Right * v * s;
}
